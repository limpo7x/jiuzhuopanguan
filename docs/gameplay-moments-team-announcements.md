## 2026-06-30 PM 公告：VID-MOMENT-001 5 秒视频留念功能计划登记

面向用户统一名称仍为“聚会记录师”。本公告仅面向 `api.pomer.cn` / `jiuzhuopanguan`，不得触碰 `pomer.cn` 公司官网。

补充执行状态：本轮已补齐本地前后端实现和视频接口 smoke，当前仍处于“待真机/待线上联调”，不能标记完成。

已落地本地改动：

- 后端：新增视频上传路由 `/api/v1/moments/uploads/video`，扩展 moment 视频合同、视频封面、时长、清理和分享图封面兼容，新增 `smoke:video-moments`。
- 前端：新增进行中页面“录 5 秒”入口、页面内 `<camera>` 自动 5 秒录制层、视频上传封装、时间线视频节点、相册视频角标和当前页视频播放弹层。
- 文档：`docs/runtime/video-moment-recording-plan-20260630.md` 已补执行派发记录。

待负责人补齐证据：

- 前端负责人：两台真机录制、上传、播放关闭、被踢/已结束/首拍前场景截图或日志。
- 后端负责人：线上上传视频接口 smoke、创建视频 moment、删除清理、分享图封面渲染证据。
- 运维负责人：`api.pomer.cn` 上传大小、OSS/CDN Range 播放、Nginx/PM2 配置证据。
- UI/UX 负责人：页面内自动倒计时录制层设计稿。
- QA：完整验收报告，不得以本地 typecheck 代替真机验收。

用户提出新增“点击后直接录制 5 秒视频并保存到时间线，可点开播放”的功能点。当前已启动本地前后端实现，尚未完成线上联调和真机验收，不得写入完成进度。

计划文档：`docs/runtime/video-moment-recording-plan-20260630.md`

范围边界：

- 首拍仍保持图片，不把视频作为开局凭证。
- 录制入口只在聚会首拍后、成员身份有效、聚会进行中时开放。
- 视频必须真实录制、真实上传、真实插入时间线，不允许图片、toast、占位节点或本地临时路径伪装成功。
- 分享长图、相册、简报只使用视频封面展示视频节点，不把视频文件嵌入长图。

协作任务：

- PM：确认视频是否带声音、每人每局数量限制、视频是否参与推举。
- UI/UX：补录制层、视频节点、播放弹层和上传态设计。
- 前端：补 live-record 录制 5 秒、上传、时间线视频节点、当前页播放预览及相册/简报/分享图兼容。
- 后端：补视频上传接口、moment 视频合同、状态判定、序列化、删除清理。
- 运维：确认 OSS/CDN 视频上传、Range 播放、Nginx 上传大小和缓存策略。
- 后台：补视频节点审核、删除和统计展示。
- QA：两台真机覆盖房主、参与者、被踢、已结束、隐私权限和上传失败场景。

当前缺口：

- 微信后台摄像头/麦克风隐私配置通过证据。
- 线上库 `moment_records` 视频字段与本地 schema 一致性证据。
- OSS/CDN 对视频文件上传和播放的验证证据。
- 录制层和视频节点设计稿。
- 前后端接口实现与联调证据。
...
## 2026-06-24 PM 公告：阶段2/3自动化复测与第四阶段保守实现

面向用户统一名称仍为“聚会记录师”。本公告仅面向 `api.pomer.cn` / `jiuzhuopanguan`，不得触碰 `pomer.cn` 公司官网。

用户要求按最新微信开发者工具自动化能力测试阶段2到阶段3，然后修改阶段4并做独立验收任务分派。

职责分派：

- 测试验收负责人：接 `VERIFY-014-03`，基于 `scripts/wechat-devtools-automator.js set-storage` 复测预览框。当前证据显示开发者工具预览包未刷新，创建页和现场记录页仍是旧编译包；只能写退回/阻断，不得写通过。
- 接口联调负责人：接 `VERIFY-014-01`，补 host/member/kicked/rejoined 线上矩阵、清理结果和残留扫描；token 只写后 8 位。
- 后台管理/后端/API 负责人：接 `VERIFY-014-02`，补真实 moment 后台通过、推举资格、公开候选、榜单、发奖、积分流水和 operationLogs 全链路；没有受控样本和清理方案前不得线上写入。
- 运营配置负责人：接 `FIX-014-19`、`FIX-014-20`、`FIX-014-24`，只维护免费模板、首页最小配置、首次登录积分和奖励空态，不新增假权益。
- 产品/运营负责人：接 `FIX-014-21`、`FIX-014-22`、`FIX-014-23`，会员/商户/分享素材保持关闭或待配置；没有真实合同、素材和发放流水前不开放领取、核销、兑换或开通。
- 前端负责人：接 `FIX-014-25` 与用户侧空态文案，继续保持旧页面未注册，不恢复旧题库或旧玩法主入口。
- PM：只汇总证据与边界；阶段2/3预览框旧包未解决前，不发布正式准出结论。

当前证据：

- 阶段2/3自动化复测：`docs/runtime/verify-fix-014-phase2-3-wechat-automation-20260624.md`。
- 第四阶段实现：`docs/runtime/pr-fix-014-phase4-ops-config-cleanup-20260624.md`。

边界：

- 当前未上传微信小程序包。
- 当前未完成阶段4部署后公网复核。
- 微信开发者工具 CLI 清理 compile 缓存被 `需要重新登录 (code 10)` 阻断；测试负责人后续需先解决开发者工具登录/编译刷新问题。
