...
const listManagedReportsFromNormalized = async (profileId = '', mode = 'all') => {
  ...
  const rows = relatedSessions.map((session) => {
    ...
    return {
      ...
      role,
      sessionId,
      sessionName: cleanText(session.name) || (report ? cleanText(reportName || reportTitle).replace(/战报$/, '') : ''),
      subtitle: cleanText(session.subtitle),
      title: cleanText(session.name) || (report ? cleanText(reportName || reportTitle).replace(/战报$/, '') : ''),
      templateName,
